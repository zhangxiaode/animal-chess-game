System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  "use strict";

  var _reporterNs, _cclegacy, GameRuleSystem, AIOpponentSystem, _crd;

  function _reportPossibleCrUseOfRankPhase(extras) {
    _reporterNs.report("RankPhase", "./RankSystem", _context.meta, extras);
  }

  function _reportPossibleCrUseOfRankTier(extras) {
    _reporterNs.report("RankTier", "./RankSystem", _context.meta, extras);
  }

  function _reportPossibleCrUseOfGameRuleSystem(extras) {
    _reporterNs.report("GameRuleSystem", "./GameRuleSystem", _context.meta, extras);
  }

  function _reportPossibleCrUseOfPosition(extras) {
    _reporterNs.report("Position", "./GameRuleSystem", _context.meta, extras);
  }

  _export("AIOpponentSystem", void 0);

  return {
    setters: [function (_unresolved_) {
      _reporterNs = _unresolved_;
    }, function (_cc) {
      _cclegacy = _cc.cclegacy;
    }, function (_unresolved_2) {
      GameRuleSystem = _unresolved_2.GameRuleSystem;
    }],
    execute: function () {
      _crd = true;

      _cclegacy._RF.push({}, "89a2fYCDSJJP6Cs+M+WkiDh", "AIOpponentSystem", undefined);

      _export("AIOpponentSystem", AIOpponentSystem = class AIOpponentSystem {
        constructor() {
          this.ruleSystem = void 0;
          this.ruleSystem = (_crd && GameRuleSystem === void 0 ? (_reportPossibleCrUseOfGameRuleSystem({
            error: Error()
          }), GameRuleSystem) : GameRuleSystem).getInstance();
        }

        static getInstance() {
          if (!AIOpponentSystem.instance) {
            AIOpponentSystem.instance = new AIOpponentSystem();
          }

          return AIOpponentSystem.instance;
        }

        chooseMove(board, aiType, playerType, tier, phase) {
          var empties = this.getEmptyCells(board);

          if (empties.length === 0) {
            return null;
          }

          var difficulty = this.getDifficulty(tier, phase);
          var candidates = [];

          for (var i = 0; i < empties.length; i++) {
            var cell = empties[i];
            var aiResult = this.simulateMove(board, cell.row, cell.col, aiType);
            var opponentMaxKill = this.getMaxKillForSide(aiResult.board, playerType);
            var centerScore = this.getCenterScore(cell.row, cell.col);
            var score = aiResult.kills * difficulty.captureWeight - opponentMaxKill * difficulty.dangerWeight + centerScore * difficulty.centerWeight;
            candidates.push({
              row: cell.row,
              col: cell.col,
              score
            });
          }

          candidates.sort((a, b) => b.score - a.score);
          var topCount = Math.min(difficulty.randomTopCount, candidates.length);
          var pickIndex = Math.floor(Math.random() * topCount);
          var picked = candidates[pickIndex];
          return {
            row: picked.row,
            col: picked.col
          };
        }

        chooseStepMove(board, aiType, playerType, tier, phase) {
          var legalMoves = this.getLegalStepMoves(board, aiType);

          if (legalMoves.length === 0) {
            return null;
          }

          var difficulty = this.getDifficulty(tier, phase);
          var candidates = [];

          for (var i = 0; i < legalMoves.length; i++) {
            var move = legalMoves[i];
            var simulatedBoard = this.cloneBoard(board);
            simulatedBoard[move.fromRow][move.fromCol] = 0;
            var aiResult = this.simulateMove(simulatedBoard, move.toRow, move.toCol, aiType);
            var opponentMaxKill = this.getMaxKillForSide(aiResult.board, playerType);
            var centerScore = this.getCenterScore(move.toRow, move.toCol);
            var score = aiResult.kills * difficulty.captureWeight - opponentMaxKill * difficulty.dangerWeight + centerScore * difficulty.centerWeight;
            candidates.push({
              fromRow: move.fromRow,
              fromCol: move.fromCol,
              toRow: move.toRow,
              toCol: move.toCol,
              score
            });
          }

          candidates.sort((a, b) => b.score - a.score);
          var topCount = Math.min(difficulty.randomTopCount, candidates.length);
          var pickIndex = Math.floor(Math.random() * topCount);
          var picked = candidates[pickIndex];
          return {
            fromRow: picked.fromRow,
            fromCol: picked.fromCol,
            toRow: picked.toRow,
            toCol: picked.toCol
          };
        }

        shouldCapture(tier, phase) {
          var level = tier * 3 + phase; // 低段位存在一定概率放弃吃子，高段位几乎必吃

          var captureChance = Math.min(0.95, 0.55 + level * 0.02);
          return Math.random() < captureChance;
        }

        getDifficulty(tier, phase) {
          var level = tier * 3 + phase; // 0 ~ 20

          return {
            captureWeight: 9 + level * 0.9,
            dangerWeight: 5 + level * 0.8,
            centerWeight: 2 + level * 0.15,
            // 低段位随机性更大，高段位更接近最优
            randomTopCount: Math.max(1, 5 - Math.floor(level / 5))
          };
        }

        simulateMove(board, row, col, pieceType) {
          var copiedBoard = this.cloneBoard(board);
          copiedBoard[row][col] = pieceType;
          var list = this.boardToPieceList(copiedBoard);
          var current = {
            point: [row, col],
            type: pieceType
          };
          var kills = this.ruleSystem.checkKill(current, list);

          for (var i = 0; i < kills.length; i++) {
            var p = kills[i];
            copiedBoard[p.point[0]][p.point[1]] = 0;
          }

          return {
            board: copiedBoard,
            kills: kills.length
          };
        }

        getMaxKillForSide(board, pieceType) {
          var empties = this.getEmptyCells(board);
          var maxKill = 0;

          for (var i = 0; i < empties.length; i++) {
            var cell = empties[i];
            var result = this.simulateMove(board, cell.row, cell.col, pieceType);

            if (result.kills > maxKill) {
              maxKill = result.kills;
            }
          }

          return maxKill;
        }

        getCenterScore(row, col) {
          // 4x4 棋盘中心偏好：中间四点分更高
          var centerTargets = [[1, 1], [1, 2], [2, 1], [2, 2]];

          for (var i = 0; i < centerTargets.length; i++) {
            if (centerTargets[i][0] === row && centerTargets[i][1] === col) {
              return 2;
            }
          }

          var edge = row === 0 || row === 3 || col === 0 || col === 3;
          return edge ? 0.5 : 1;
        }

        getEmptyCells(board) {
          var cells = [];

          for (var row = 0; row < board.length; row++) {
            for (var col = 0; col < board[row].length; col++) {
              if (board[row][col] === 0) {
                cells.push({
                  row,
                  col
                });
              }
            }
          }

          return cells;
        }

        getLegalStepMoves(board, pieceType) {
          var moves = [];
          var dirs = [[-1, 0], [1, 0], [0, -1], [0, 1]];

          for (var row = 0; row < board.length; row++) {
            for (var col = 0; col < board[row].length; col++) {
              if (board[row][col] !== pieceType) continue;

              for (var i = 0; i < dirs.length; i++) {
                var nextRow = row + dirs[i][0];
                var nextCol = col + dirs[i][1];
                if (nextRow < 0 || nextRow >= 4 || nextCol < 0 || nextCol >= 4) continue;
                if (board[nextRow][nextCol] !== 0) continue;
                moves.push({
                  fromRow: row,
                  fromCol: col,
                  toRow: nextRow,
                  toCol: nextCol
                });
              }
            }
          }

          return moves;
        }

        boardToPieceList(board) {
          var list = [];

          for (var row = 0; row < board.length; row++) {
            for (var col = 0; col < board[row].length; col++) {
              var type = board[row][col];

              if (type !== 0) {
                list.push({
                  point: [row, col],
                  type
                });
              }
            }
          }

          return list;
        }

        cloneBoard(board) {
          var copied = [];

          for (var i = 0; i < board.length; i++) {
            copied[i] = [];

            for (var j = 0; j < board[i].length; j++) {
              copied[i][j] = board[i][j];
            }
          }

          return copied;
        }

      });

      AIOpponentSystem.instance = void 0;

      _cclegacy._RF.pop();

      _crd = false;
    }
  };
});
//# sourceMappingURL=a103d0ae56e0d5bdd978d14fe4d3a0b011e51561.js.map