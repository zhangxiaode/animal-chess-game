System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function addToSidebar() {
    ...
    var api = platform === 'wechat' ? runtime.wx : platform === 'douyin' ? runtime.tt : null;

    if (!api) {
      var message = '当前环境不支持添加侧边栏';
    ...
    var candidates = platform === 'wechat' || platform === 'douyin' ? SIDEBAR_CANDIDATES[platform] : [];
    var candidate = candidates.find(item => typeof api[item.method] === 'function');

    if (!candidate) {
      var _message = platform + " \u5F53\u524D\u57FA\u7840\u5E93\u4E0D\u652F\u6301\u6DFB\u52A0\u4FA7\u8FB9\u680F";

      console.warn("[SidebarUtils] " + _message);
      showToast(runtime, '暂不支持');
      return Promise.resolve({
        platform,
        success: false,
  ...
  function callSidebarApi(platform, api, candidate) {
    return new Promise(resolve => {
      ...
      try {
        api[candidate.method](_extends({}, candidate.options, {
          success: res => {
            var message = '添加侧边栏成功';
            clearTimeout(timer);
  ...
  function showToast(runtime, title) {
    ...
    var api = (_runtime$wx = runtime.wx) != null ? _runtime$wx : runtime.tt;
