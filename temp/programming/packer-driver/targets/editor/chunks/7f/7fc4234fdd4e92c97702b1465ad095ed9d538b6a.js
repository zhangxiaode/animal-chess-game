    ...
    execute: function () {
      ...
      _export("GameRuleSystem", GameRuleSystem = class GameRuleSystem {
        ...
        checkKill(current, oldList) {
          ...
          const rivalPoints = oldList.filter(item => item.type !== current.type); // 自己剩余棋子

          const selfPoints = oldList.filter(item => item.type === current.type); // 空余棋位

          const emptyPoints = allPoints.filter(item => oldList.every(child => !(item[0] === child.point[0] && item[1] === child.point[1])) && !(item[0] === current.point[0] && item[1] === current.point[1])); // 获取所有可能的吃子规则

          const allKills = this.getAllKillPatterns(current); // 过滤满足条件的规则

          const matchedKills = this.filterKills(allKills, selfPoints, rivalPoints, emptyPoints); // 提取被吃掉的棋子

          const killedPieces = [];

          for (const killPattern of matchedKills) {
            for (const rivalPoint of killPattern.rival) {
              const killedPiece = oldList.find(item => item.point[0] === rivalPoint[0] && item.point[1] === rivalPoint[1]);
...
//# sourceMappingURL=7fc4234fdd4e92c97702b1465ad095ed9d538b6a.js.map