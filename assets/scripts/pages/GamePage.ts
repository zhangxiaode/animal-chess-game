...
export class GamePage extends Component {
    ...
    private _capturePopupRoot: Node | null = null;
    private _capturePopupMessage: Label | null = null;
    private _pendingCaptureDecision: ((shouldCapture: boolean) => void) | null = null;
     ...
     * 创建吃子确认弹框
     */
    private _createCapturePopup(parent: Node) {
        const pageTransform = parent.getComponent(UITransform);
        const pageSize = pageTransform?.contentSize;
        const pageWidth = pageSize?.width ?? 640;
        const pageHeight = pageSize?.height ?? 960;
    ...
    private _drawGameBoard() {
        ...
        const userPanelHalfHeight = (this._userInfoRoot?.getComponent(UITransform)?.contentSize.height ?? this._scaled(120, uiScale)) / 2;
        const opponentPanelHalfHeight = (this._opponentInfoRoot?.getComponent(UITransform)?.contentSize.height ?? this._scaled(120, uiScale)) / 2;
    ...
    private _onBoardTouch(event: EventTouch) {
        if (this._isGameOver || this._isAiThinking || this._currentTurn !== 1 || !this._boardRoot || this._discardPopupRoot?.active || this._capturePopupRoot?.active) {
            return;
        }
     ...
     * 执行吃子决策（玩家可选择吃或不吃）
     */
    private _resolveCaptureAfterAction(
        row: number,
        col: number,
        pieceType: number,
        isPlayer: boolean,
    ...
    private _showCapturePopup(killCount: number, onDecision: (shouldCapture: boolean) => void) {
